...
void File::chomp(std::string & line)
{
    ...
    //line = regex_replace(line, regex("[[:space:]]+$"), std::string(""));
}
#else
void File::chomp(std::string & line)
{
...
void File::readSignal(Network & network, Message & message, std::string & line)
{
    ...
    if (regex_search(line, m, re)) {
        ...
        Signal & signal = message.signal[signalName];
...
void File::readMessageTransmitter(Network & network, std::string & line)
{
    ...
    if (regex_search(line, m, re)) {
        ...
        while(iss.good()) {
            ...
            iss >> transmitter;
#ifdef F_DEBUG
            std::cout << "insert transmitter " << transmitter << endl;
#endif
            message.transmitters.insert(transmitter);
        }
#else
        /*
         * The original code deals with transmitters split with whitespace, but
         * we has ',' there, e.g.
         *
         * BO_TX_BU_ 201 : ECM_PB,CGM_PB;
         */
        std::string pattern = ",";
        std::vector<std::string> transmitter = split(m[2], pattern);

        for(unsigned int i = 0; i < transmitter.size(); i++)
        {
            message.transmitters.insert(transmitter[i]);
        }

        // set transmitter to null so that we know there are more than one trx
        message.transmitter = "";
#endif
...
void File::readSignalType(Network & network, std::string & line)
{
    ...
    if (regex_search(line, mSTR, reSTR)) {
        ...
        network.messages[messageId].signal[signalName].type = signalTypeName;
        return;
    }
...
bool File::readCommentSignal(Network & network, std::stack<std::size_t> & lineBreaks, std::string & line)
{
    ...
    if (regex_search(line, m, re)) {
        ...
        network.messages[messageId].signal[signalName].comment = comment;
        return true;
    }
...
bool File::readAttributeValueSignal(Network & network, std::string & line)
{
    ...
    if (regex_search(line, m, re)) {
        ...
        AttributeDefinition & attributeDefinition = network.attributeDefinitions[attributeName];
        Attribute & attribute = network.messages[messageId].signal[signalName].attributeValues[attributeName];
...
bool File::readValueDescriptionSignal(Network & network, std::string & line)
{
    ...
    if (regex_search(line, m, re)) {
        unsigned int messageId = stoul(m[1]);
        std::string signalName = m[2];
        ValueDescriptions & valueDescriptions = network.messages[messageId].signal[signalName].valueDescriptions;
...
void File::readSignalGroup(Network & network, std::string & line)
{
    ...
    if (regex_search(line, m, re)) {
        ...
        while (iss.good()) {
            ...
            iss >> signalName;
            signalGroup.signal.insert(signalName);
        }
    }
...
void File::readSignalExtendedValueType(Network & network, std::string & line)
{
    ...
    if (regex_search(line, m, re)) {
        ...
        // Integer
        case 0:
            network.messages[messageId].signal[signalName].extendedValueType = Signal::ExtendedValueType::Integer;
            break;
        // Float
        case 1:
            network.messages[messageId].signal[signalName].extendedValueType = Signal::ExtendedValueType::Float;
            break;
        // Double
        case 2:
            network.messages[messageId].signal[signalName].extendedValueType = Signal::ExtendedValueType::Double;
            break;
        }
        return;
    }
...
void File::readExtendedMultiplexor(Network & network, std::string & line)
{
    ...
    if (regex_search(line, m, re)) {
        ...
        ExtendedMultiplexor & extendedMultiplexor = network.messages[messageId].signal[multiplexedSignalName].extendedMultiplexors[switchName];
